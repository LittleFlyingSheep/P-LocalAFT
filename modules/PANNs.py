...
class Cnn10(nn.Module):
    ...
    def __init__(self, spec_aug):

        super(Cnn10, self).__init__()

        self.bn0 = nn.BatchNorm2d(64)

        self.is_spec_augment = spec_aug
    ...
    def forward(self, input, mixup_param=None):
        ...
        x = F.dropout(x, p=0.2, training=self.training)
        x = F.relu_(self.fc2(x))  # time x batch x channel (128)
        return x
...
if __name__ == '__main__':
    model = Cnn10(spec_aug=True)
    model_dict = model.state_dict()
    dict = torch.load('./Cnn10.pth')['model']
    dict = {k: v for k, v in dict.items() if k in model_dict}
    model.load_state_dict(dict, strict=False)
    torch.save(model.state_dict(), './CNN10_encoder.pth')
    print(model)
