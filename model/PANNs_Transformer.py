...
class Seq2SeqMLPConvTransformer(nn.Module):
    def __init__(self, frequency_dim, hidden_dim, emb_size=256, dim_feedforward=512, dropout=.2,
                 num_decoder_layers=3, spec_aug=False, nb_classes=None):
    ...
    def init_vars(self, src, k_beam, max_steps=22, with_pad=False):
        ...
        # k_memorys = torch.zeros(k_beam, memory.shape[-2], memory.shape[-1]).to(device)
    ...
    def beam_search(self, src, k_beam=5, max_steps=22, with_pad=False):
        # to do init
        # src = src.transpose(0, 1).contiguous()   # (Seq, B)
        ...
        # ys = F.one_hot(ys.unsqueeze(0), 4368).float()
        # print(ys.shape)
        return ys
